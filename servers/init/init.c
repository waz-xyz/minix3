...
int main(void)
{
	...
	else
	{
		...
		if (svrctl(MMGETPARAM, &sysgetenv) == 0)
			*rcp++ = bootopts;
		*rcp = "start";
	...
	while (1)
	{
		...
		if (spawn && check)
		{
			...
			for (linenr = 0; linenr < PIDSLOTS; linenr++)
			{
				...
				if (ttyp->ty_getty != NULL && ttyp->ty_getty[0] != NULL && slotp->pid == NO_PID && slotp->errct < ERRCT_DISABLE)
				{
					startup(linenr, ttyp);
				}
			}
...
)
{
	...
	case DEAD_PROCESS:
		...
		if (lseek(fd, (off_t)(linenr + 1) * sizeof(utmp), SEEK_SET) == -1 || read(fd, &utmp, sizeof(utmp)) == -1)
		{
			report(2, PATH_UTMP);
			close(fd);
			return;
	...
	case DEAD_PROCESS:
		...
		if ((fd = open(PATH_UTMP, O_WRONLY)) < 0 || lseek(fd, (off_t)(linenr + 1) * sizeof(utmp), SEEK_SET) == -1 || write(fd, &utmp, sizeof(utmp)) == -1)
		{
			if (errno != ENOENT)
				report(2, PATH_UTMP);
		}
	...
	case DEAD_PROCESS:
		/* Add new wtmp entry. */
		if ((fd = open(PATH_WTMP, O_WRONLY | O_APPEND)) < 0 || write(fd, &utmp, sizeof(utmp)) == -1)
		{
			if (errno != ENOENT)
				report(2, PATH_WTMP);
		}
