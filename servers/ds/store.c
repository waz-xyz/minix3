...
PRIVATE int set_owner(struct data_store *dsp, void *auth_ptr);
PRIVATE int is_authorized(struct data_store *dsp, void *auth_ptr);

PRIVATE int set_owner(
...
)
{
	...
	*dsp = NULL;
	...
	{
		if ((ds_store[i].ds_flags & DS_IN_USE) != 0 && ds_store[i].ds_key == key)
		{
			*dsp = &ds_store[i];
...
)
{
	...
	{
		if (nr_in_use >= NR_DS_KEYS)	/* store is full */
		{
			return EAGAIN;
		}
		else				/* new slot found */
