...
FORWARD void cleanup(register struct mproc *child);
...
/* The process pointed to by 'mp' has forked.  Create a child process. */
{
	...
	if (next_child < 0 || next_child >= NR_PROCS || (mproc[next_child].mp_flags & IN_USE))
 ...
 */
{
	...
	if (rmp->mp_flags & ALARM_ON)
		set_alarm(proc_nr_e, (unsigned)0);
	...
	right_child = (pidarg == -1 || pidarg == rmp->mp_pid || -pidarg == rmp->mp_procgrp);
	...
	{
		if ((rmp->mp_flags & IN_USE) != 0 && rmp->mp_parent == proc_nr)
		{
