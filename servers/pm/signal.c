...
FORWARD void dump_core(struct mproc *rmp);
FORWARD void unpause(int pro);
FORWARD void handle_ksig(int proc_nr, sigset_t sig_map);
FORWARD void cause_sigalrm(struct timer *tp);

/*===========================================================================*
 *				do_sigaction				     *
 *===========================================================================*/
PUBLIC int do_sigaction(void)
{
	int r;
 ...
 *				do_sigprocmask                               *
 *===========================================================================*/
PUBLIC int do_sigprocmask(void)
 ...
 *				handle_ksig				     *
 *===========================================================================*/
PRIVATE void handle_ksig(int proc_nr_e, sigset_t sig_map)
{
 ...
 *				cause_sigalrm				     *
 *===========================================================================*/
PRIVATE void cause_sigalrm(struct timer *tp)
{
 ...
 */
{
	...
	for (rmp = &mproc[0]; rmp < &mproc[NR_PROCS]; rmp++)
	{
		if (!(rmp->mp_flags & IN_USE))
			continue;
		if ((rmp->mp_flags & ZOMBIE) && signo != 0)
			continue;
	...
	if ((mp->mp_flags & (IN_USE | ZOMBIE)) != IN_USE)
		return SUSPEND;
 ...
 *				check_pending				     *
 *===========================================================================*/
PUBLIC void check_pending(struct mproc *rmp)
...
PRIVATE void dump_core(
	struct mproc *rmp	/* whose core is to be dumped */
)
/* Make a core dump on the file "core", if possible. */
{
	...
	if (write(fd, (char *)rmp->mp_seg, (unsigned)sizeof rmp->mp_seg) != (unsigned)sizeof rmp->mp_seg)
	{
		close(fd);
		return;
	}
