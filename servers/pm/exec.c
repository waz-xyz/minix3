...
FORWARD int new_mem(struct mproc * sh_mp, vir_bytes text_bytes,
...
FORWARD int insert_arg(char stack[ARG_MAX], vir_bytes *stk_bytes, char *arg, int replace);
FORWARD char *patch_stack(int fd, char stack[ARG_MAX], vir_bytes *stk_bytes, char *script);
FORWARD int read_header(int fd, int *ft, vir_bytes *text_bytes,
			vir_bytes *data_bytes, vir_bytes *bss_bytes,
 ...
 */
{
	...
	if (stk_bytes > ARG_MAX)
		return ENOMEM;		/* stack too big */
	if (m_in.exec_len <= 0 || m_in.exec_len > PATH_MAX)
		return EINVAL;
...
/* Read the header and extract the text, data, bss and total sizes from it. */
{
	...
	if ((m = read(fd, &hdr, A_MINHDR)) < 2)
		return ENOEXEC;

	/* Interpreted script? */
	if (((char *)&hdr)[0] == '#' && ((char *)&hdr)[1] == '!')
		return ESCRIPT;
	...
	*pc = hdr.a_entry;		/* initial address to start execution */
	...
	if (ct > A_MINHDR)
		lseek(fd, (off_t)ct, SEEK_SET);	/* skip unused hdr */
	return m;
}
 ...
 */
{
	...
	if (!insert_arg(stack, stk_bytes, script, REPLACE))
		return NULL;
 ...
 *				find_share				     *
 *===========================================================================*/
PUBLIC struct mproc *find_share(
