 ...
 *				del_slot				     *
 *===========================================================================*/
PRIVATE void del_slot(
	struct hole *prev_ptr,	/* pointer to hole entry just ahead of 'hp' */
	struct hole *hp		/* pointer to hole entry to be removed */
)
 ...
 *===========================================================================*/
PRIVATE void merge(
	struct hole *hp		/* ptr to hole to merge with its successors */
)
...
PUBLIC int swap_on(
	...
	u32_t offset, u32_t size	/* area on swap file to use */
)
/* Turn swapping on. */
{
	if (swap_fd != -1)
		return EBUSY;			/* already have swap? */

	tell_fs(CHDIR, who_e, FALSE, 0);	/* be like the caller for open() */
	if ((swap_fd = open(file, O_RDWR)) < 0)
		return -errno;
	swap_offset = offset;
	size >>= CLICK_SHIFT;
	if (size > swap_maxsize)
		size = swap_maxsize;
	if (size > 0)
		free_mem(swap_base, (phys_clicks)size);
	return OK;
}
...
/* Turn swapping off. */
{
	...
	if (swap_fd == -1)
		return OK;	/* can't turn off what isn't on */
 ...
 *				swap_inqueue				     *
 *===========================================================================*/
PUBLIC void swap_inqueue(
	struct mproc *rmp		/* process to add to the queue */
)
 ...
 */
{
	...
	while ((rmp = *pmp) != NULL)
	{
		...
		if ((rmp->mp_flags & SWAPIN) == 0)
		{
			/* Guess it got killed. (Queue is cleaned here.) */
			*pmp = rmp->mp_swapq;
			continue;
		}
		else if ((new_base = alloc_mem(size)) == NO_MEM)
		{
 ...
 */
{
	...
	do
	{
		...
		/* A candidate? */
		if ((rmp->mp_flags & (PAUSED | WAITING | SIGSUSPENDED)) == 0)
			continue;

		/* Already on swap or otherwise to be avoided? */
		if ((rmp->mp_flags & (DONT_SWAP | TRACED | REPLY | ONSWAP)) != 0)
			continue;
