...
typedef struct pty
{
	...
	int select_ready_ops; /* For callback. */
} pty_t;
...
/* Perform an open/close/read/write call on a /dev/ptypX device. */
{
	...
	case DEV_READ:
		...
		{
#else
		if ((r = sys_umap(m_ptr->IO_ENDPT, (vir_bytes)m_ptr->ADDRESS,
				  m_ptr->COUNT, NULL)) != OK)
		{
#endif
			break;
 ...
 */
{
	...
	for (;;)
	{
		...
		if ((s = sys_vircopy(tp->tty_outproc, (vir_bytes)tp->tty_out_vir,
				     SELF, (vir_bytes)pp->ohead,
				     (phys_bytes)count)) != OK)
		{
...
/* Transfer bytes written to the output buffer to the PTY reader. */
{
	...
	for (;;)
	{
		...
		if ((s = sys_vircopy(SELF, (vir_bytes)pp->otail,
				     (vir_bytes)pp->rdproc, (vir_bytes)pp->rdvir,
				     (phys_bytes)count)) != OK)
		{
			printf("pty tty%d: copy failed (error %d)\n", s);
			break;
		...
		/* Bookkeeping. */
		pp->ocount -= count;
		if ((pp->otail += count) == bufend(pp->obuf))
			pp->otail = pp->obuf;
		pp->rdvir += count;
		pp->rdcum += count;
		pp->rdleft -= count;
 ...
 */
{
	...
	while (pp->wrleft > 0)
	{
		...
		if ((s = sys_vircopy(pp->wrproc, (vir_bytes)pp->wrvir,
				     SELF, (vir_bytes)&c,
				     (phys_bytes)1)) != OK)
		{
...
PRIVATE int select_try_pty(tty_t *tp, int ops)
{
	...
	if (ops & SEL_WR)
	{
		...
		if (pp->state & TTY_CLOSED)
			r |= SEL_WR;
		else if (pp->wrleft != 0 || pp->wrcum != 0)
			r |= SEL_WR;
		else
			r |= SEL_WR;
	}
	...
	if (ops & SEL_RD)
	{
		...
		if (pp->state & TTY_CLOSED)
			r |= SEL_RD;
		else if (pp->rdleft != 0 || pp->rdcum != 0)
			r |= SEL_RD;
		else if (pp->ocount > 0)
			r |= SEL_RD; /* Actual data. */
	}
