...
FORWARD int cons_ioctl(tty_t *tp, int try);
PRIVATE void ser_putc(char c);

/*===========================================================================*
 *				cons_write				     *
 *===========================================================================*/
PRIVATE int cons_write(
    struct tty *tp, /* tells which terminal is to be used */
    int try
)
 ...
 */
{
   ...
   */
	do
	{
		if (count > sizeof(buf))
			count = sizeof(buf);
		if ((result = sys_vircopy(tp->tty_outproc, tp->tty_out_vir,
					  SELF, (vir_bytes)buf,
					  (vir_bytes)count)) != OK)
			break;
		tbuf = buf;
	 ...
	 */
		do
		{
			if ((unsigned)*tbuf < ' ' || cons->c_esc_state > 0 || cons->c_column >= scr_width || cons->c_rwords >= buflen(cons->c_ramqueue))
			{
				out_char(cons, *tbuf++);
			}
			else
...
PRIVATE void out_char(
    console_t *cons, /* pointer to console struct */
...
/* Output a character on the console.  Check for escape sequences first. */
{
	...
	case '\n': /* line feed */
		if ((cons->c_tty->tty_termios.c_oflag & (OPOST | ONLCR)) == (OPOST | ONLCR))
		{
			cons->c_column = 0;
		}
		/*FALL THROUGH*/
	...
	default: /* printable chars are stored in ramqueue */
		...
		if (cons->c_rwords == buflen(cons->c_ramqueue))
			flush(cons);
...
)
{
	...
	else if (cons->c_esc_intro == '[')
	{
		...
		case 'm': /* ESC [nm enables rendition n */
			for (parmp = cons->c_esc_parmv; parmp <= cons->c_esc_parmp && parmp < bufend(cons->c_esc_parmv); parmp++)
			{
				if (cons->c_reverse)
				{
...
PUBLIC void do_video(message *m)
{
	...
	case DEV_IOCTL:
		if (m->TTY_REQUEST == MIOCMAP || m->TTY_REQUEST == MIOCUNMAP)
		{
			...
			r = sys_vircopy(m->IO_ENDPT, (vir_bytes)m->ADDRESS,
					SELF, (vir_bytes)&mapreq,
					sizeof(mapreq));
			if (r != OK)
			{
...
/* Initialize the screen driver. */
{
	...
	if (!vdu_initialized++)
	{
		...
		// 		SELF, (vir_bytes)&bios_columns, VDU_SCREEN_COLS_SIZE);
		// s = sys_vircopy(SELF, BIOS_SEG, (vir_bytes)VDU_CRT_BASE_ADDR,
		// 		SELF, (vir_bytes)&bios_crtbase, VDU_CRT_BASE_SIZE);
		// s = sys_vircopy(SELF, BIOS_SEG, (vir_bytes)VDU_SCREEN_ROWS_ADDR,
		// 		SELF, (vir_bytes)&bios_rows, VDU_SCREEN_ROWS_SIZE);
		// s = sys_vircopy(SELF, BIOS_SEG, (vir_bytes)VDU_FONTLINES_ADDR,
		// 		SELF, (vir_bytes)&bios_fontlines, VDU_FONTLINES_SIZE);
...
{
	if (cons_line < 0 || cons_line >= nr_cons)
		return;
	ccurrent = cons_line;
