...
#if (MACHINE != IBM_PC) && (MACHINE != ATARI) && (MACHINE != ZYNQ)
#error rs232.c only supports PC, Atari ST and Zynq
#endif
...
#elif (MACHINE == ATARI) /* Atari ST 68901 USART */
...
uint32_t dummy_regs[256];

enum
{
	UART0_BASE		= 0xE0000000U,
	UART1_BASE		= 0xE0001000U,
	XUARTPS_CR_OFFSET	= 0x00,
	XUARTPS_MR_OFFSET	= 0x04,
	XUARTPS_IER_OFFSET	= 0x08,
	XUARTPS_IDR_OFFSET	= 0x0C,
	XUARTPS_IMR_OFFSET	= 0x10,
	XUARTPS_ISR_OFFSET	= 0x14,
	XUARTPS_BAUDGEN_OFFSET	= 0x18,
	XUARTPS_RXTOUT_OFFSET	= 0x1C,
	XUARTPS_RXWM_OFFSET	= 0x20,
	XUARTPS_MODEMCR_OFFSET	= 0x24,
	XUARTPS_MODEMSR_OFFSET	= 0x28,
	XUARTPS_SR_OFFSET	= 0x2C,
	XUARTPS_FIFO_OFFSET	= 0x30,
	XUARTPS_BAUDDIV		= 0x34,
	XUARTPS_FLOWDEL		= 0x38,
	XUARTPS_TXWM_OFFSET	= 0x44,
	XUARTPS_RXBS		= 0x48,
	XUARTPS_TI		= (1U << 10),
	XUARTPS_MS		= (1U << 9),
	XUARTPS_RTR		= (1U << 0)
...
#define istart(rs)                                  \
	{                                           \
		set_porta(~(PA_SRTS | PA_SDTR), 0); \
		(rs)->idevready = TRUE;             \
	}
#define istop(rs)                             \
	{                                     \
		set_porta(~PA_SDTR, PA_SRTS); \
		(rs)->idevready = FALSE;      \
	}
...
#define istart(rs)	
#define istop(rs)
#define devready(rs)	(0)
#define txready(rs)	((((rs)->uart_regs[XUARTPS_SR_OFFSET]) & (1 << 4)) == 0)
#define devhup(rs)	(0)
 ...
 *				rs_write				     *
 *===========================================================================*/
PRIVATE int rs_write(tty_t *tp, int try)
/* (*devwrite)() routine for RS232. */
{
	...
	for (;;)
	{
		...
		sys_vircopy(tp->tty_outproc, (vir_bytes)tp->tty_out_vir,
			    SELF, (vir_bytes)rs->ohead,
			    (phys_bytes)count);
 ...
 */
{
	...
	{
		if (s2dp->speed == tp->tty_termios.c_ospeed)
			divisor = s2dp->divisor;
	}
	if (divisor == 0)
		return; /* B0? */
	...
	if (tp->tty_termios.c_cflag & PARENB)
	{
		...
		if (!(tp->tty_termios.c_cflag & PARODD))
			line_controls |= LC_PAREVEN;
	}
	if (divisor >= (UART_FREQ / 110))
		line_controls |= LC_2STOP_BITS;
...
/* Initialize RS232 for one line. */
{
...
#elif (MACHINE == ZYNQ)
	// assert(sys_vm_map(SELF, TRUE,
	// 		  line == 0 ? UART0_BASE : UART1_BASE,
	// 		  SMALL_PAGE_SIZE, 0, (vir_bytes*)&rs->uart_regs) == OK);
	rs->uart_regs = dummy_regs;
#endif
	...
	sys_outb(rs->int_enab_port, IE_LINE_STATUS_CHANGE | IE_MODEM_STATUS_CHANGE | IE_RECEIVER_READY | IE_TRANSMITTER_READY);
#elif (MACHINE == ATARI)
...
#elif (MACHINE == ZYNQ)
	irq = line == 0 ? IRQ_UART0 : IRQ_UART1;
	irqs[line] = irq;
	rs->irq = irq;
	rs->irq_hook_id = rs->irq; /* call back with irq line number */
	if ((res = sys_irqsetpolicy(irq, IRQ_REENABLE, &rs->irq_hook_id)) != OK)
	{
		printf("RS232: Couldn't obtain hook for irq %d (res = %d)\n", irq, res);
	}
	else if ((res = sys_irqenable(&rs->irq_hook_id)) != OK)
	{
		printf("RS232: Couldn't enable irq %d (res = %d)\n", irq, res);
	}

	printf("rs_init: rs->uart_regs = 0x%08X\n", rs->uart_regs);
	rs->uart_regs[XUARTPS_ISR_OFFSET] = 0;
	rs->uart_regs[XUARTPS_RXWM_OFFSET] = 1;
	rs->uart_regs[XUARTPS_TXWM_OFFSET] = 1;
	rs->uart_regs[XUARTPS_IER_OFFSET] = XUARTPS_TI | XUARTPS_MS | XUARTPS_RTR;
	printf("rs_init: rs->uart_regs were configured\n");
#endif /* MACHINE == IBM_PC */
	...
	istart(rs);
	printf("rs_init: complete\n");
}

/*===========================================================================*
 *				rs_interrupt				     *
 *===========================================================================*/
PUBLIC void rs_interrupt(
	message *m			/* which TTY */
)
{
	int irq;
...
#elif (MACHINE == ZYNQ)
	line = tp - &tty_table[NR_CONS];
	assert(sys_vm_map(SELF, FALSE,
			  line == 0 ? UART0_BASE : UART1_BASE,
			  SMALL_PAGE_SIZE, (vir_bytes)rs->uart_regs, NULL) == OK);
	irqs[line] = 0;
#endif /* MACHINE == IBM_PC */
	return 0;	/* dummy */
}
 ...
 *				rs232_handler				     *
 *===========================================================================*/
PRIVATE void rs232_handler(rs) struct rs232 *rs;
{
 ...
 *				siaint					     *
 *===========================================================================*/
PRIVATE void siaint(type) int type; /* interrupt type */
{
...
/* Interrupt handler for RS232. */
{
	while (TRUE)
	{
		 ...
		 * Unfortunately, some serial cards lock up without this.
		 */
		v = rs->uart_regs[XUARTPS_ISR_OFFSET];
		if ((v & XUARTPS_RTR) != 0)
		{
			in_int(rs);
			continue;
		}
		else if ((v & XUARTPS_TI) != 0)
		{
			out_int(rs);
			continue;
		}
		else if ((v & XUARTPS_MS) != 0)
		{
			modem_int(rs);
			continue;
		}
		// else if (IS_LINE_STATUS_CHANGE)
		// {
		// 	line_int(rs);
		// 	continue;
		// }
 ...
 *===========================================================================*/
PRIVATE void in_int(
	rs232_t *rs			/* line with input interrupt */
)
 ...
 */
{
	...
	c = MFP->mf_udr;
#elif (MACHINE == ZYNQ)
	c = rs->uart_regs[XUARTPS_FIFO_OFFSET];
#endif /* MACHINE == IBM_PC */
	if (!(rs->ostate & ORAW))
	{
		if (c == rs->oxoff)
 ...
 *				line_int				     *
 *===========================================================================*/
PRIVATE void line_int(
	rs232_t *rs		/* line with line status interrupt */
)
...
#endif /* MACHINE == IBM_PC */
	// if (rs->lstatus & LS_FRAMING_ERR)
	// 	++rs->framing_errors;
	// if (rs->lstatus & LS_OVERRUN_ERR)
	// 	++rs->overrun_errors;
	// if (rs->lstatus & LS_PARITY_ERR)
	// 	++rs->parity_errors;
	// if (rs->lstatus & LS_BREAK_INTERRUPT)
	// 	++rs->break_interrupts;
}
 ...
 *				modem_int				     *
 *===========================================================================*/
PRIVATE void modem_int(
	rs232_t *rs		/* line with modem interrupt */
)
 ...
 *				out_int					    *
 *===========================================================================*/
PRIVATE void out_int(
	rs232_t *rs		/* line with output interrupt */
)
 ...
 */
{
	if (rs->ostate >= (ODEVREADY | OQUEUED | OSWREADY))
	{
		...
		MFP->mf_udr = *rs->otail;
#elif (MACHINE == ZYNQ)
		rs->uart_regs[XUARTPS_FIFO_OFFSET] = *rs->otail;
#endif
		...
		else if (rs->ocount == RS_OLOWWATER) /* running low? */
		{
			rs->tty->tty_events = 1;
			force_timeout();
		}
...
PUBLIC int rs_is_valid_interrupt(int irq)
{
