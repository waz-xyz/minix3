...
FORWARD void micro_delay(unsigned long usecs);
 ...
 *				handle_req				     *
 *===========================================================================*/
PRIVATE void handle_req(struct kbd *kbdp, message *m)
{
	int i, n, r, ops, watch;
	unsigned char c;
 ...
 *				handle_status				     *
 *===========================================================================*/
PRIVATE int handle_status(struct kbd *kbdp, message *m)
{
	int n, r;
	...
	if (kbdp->avail && kbdp->req_size && m->m_source == kbdp->incaller)
	{
		...
		kbdp->req_size = 0;
		r = sys_vircopy(SELF, (vir_bytes)&kbdp->buf[kbdp->offset],
				kbdp->req_proc, kbdp->req_addr,
				n);
		if (r == OK)
		{
			kbdp->offset = (kbdp->offset + n) % KBD_BUFSZ;
...
/* Map a scan code to an ASCII code. */
{
	...
	caps = shift;
	lk = locks[ccurrent];
	if ((lk & NUM_LOCK) && HOME_SCAN <= scode && scode <= DEL_SCAN)
		caps = !caps;
	if ((lk & CAPS_LOCK) && (keyrow[0] & HASCAPS))
		caps = !caps;
 ...
 *				kbc_cmd1				     *
 *===========================================================================*/
PRIVATE void kbc_cmd1(int cmd, int data)
{
	kb_wait();
	sys_outb(KB_COMMAND, cmd);
	kb_wait();
...
{
	int result;
	result = sys_vircopy(m->IO_ENDPT, (vir_bytes)m->ADDRESS,
			     SELF, (vir_bytes)keymap,
 ...
 *				show_key_mappings			     *
 *===========================================================================*/
PRIVATE void show_key_mappings(void)
{
	int i, s;
	struct proc proc;
 ...
 *				scan_keyboard				     *
 *===========================================================================*/
PRIVATE int scan_keyboard(unsigned char *bp, int *isauxp)
{
 ...
 *				kbd_watchdog 				     *
 *===========================================================================*/
PRIVATE void kbd_watchdog(timer_t *tmrp)
{
	int r;
	clock_t now;
