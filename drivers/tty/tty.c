...
FORWARD void expire_timers(void);
...
#if ENABLE_SRCCOMPAT || ENABLE_BINCOMPAT
FORWARD int compat_getp(tty_t *tp, struct sgttyb *sg);
FORWARD int compat_getc(tty_t *tp, struct tchars *sg);
FORWARD int compat_setp(tty_t *tp, struct sgttyb *sg);
FORWARD int compat_setc(tty_t *tp, struct tchars *sg);
FORWARD int tspd2sgspd(speed_t tspd);
FORWARD speed_t sgspd2tspd(int sgspd);
#if ENABLE_BINCOMPAT
FORWARD void do_ioctl_compat(tty_t *tp, message *m_ptr);
#endif
#endif
...
/* Main routine of the terminal task. */
{
	...
	while (TRUE)
	{
		...
		for (tp = FIRST_TTY; tp < END_TTY; tp++)
		{
			if (tp->tty_events)
			{
				printf("tty: Handling events for tty %d\n", tp-FIRST_TTY);
				handle_events(tp);
			}
		}

		/* Get a request message. */
		printf("tty: About to receive a message");
...
#endif	/* MACHINE == IBM_PC */
#if NR_RS_LINES > 0
			if (rs_is_valid_interrupt(tty_mess.NOTIFY_ARG))
...
/* A process wants to read from a terminal. */
{
	...
	*/
	...
	else if (sys_umap(m_ptr->IO_ENDPT, (vir_bytes)m_ptr->ADDRESS, m_ptr->COUNT, NULL) != OK)
	{
		if (more_verbose)
			printf("do_read: EFAULT\n");
		r = EFAULT;
	}
	else
	{
 ...
 */
{
	...
	case TCGETS:
		...
		r = sys_vircopy(SELF, (vir_bytes)&tp->tty_termios,
				m_ptr->IO_ENDPT, (vir_bytes)m_ptr->ADDRESS,
				(vir_bytes)size);
		break;
	...
	case TCDRAIN:
		...
		if (m_ptr->TTY_REQUEST == TCDRAIN)
			break;
		if (m_ptr->TTY_REQUEST == TCSETSF)
			tty_icancel(tp);
		/*FALL THROUGH*/
	case TCSETS:
		/* Set the termios attributes. */
		r = sys_vircopy(m_ptr->IO_ENDPT, (vir_bytes)m_ptr->ADDRESS,
				SELF, (vir_bytes)&tp->tty_termios, (vir_bytes)size);
		if (r != OK)
			break;
		setattr(tp);
		break;
	...
	case TCFLSH:
		r = sys_vircopy(m_ptr->IO_ENDPT, (vir_bytes)m_ptr->ADDRESS,
				SELF, (vir_bytes)&param.i, (vir_bytes)size);
		if (r != OK)
			break;
		switch (param.i)
		{
	...
	case TIOCGWINSZ:
		r = sys_vircopy(SELF, (vir_bytes)&tp->tty_winsize,
				m_ptr->IO_ENDPT, (vir_bytes)m_ptr->ADDRESS,
				(vir_bytes)size);
		break;

	case TIOCSWINSZ:
		printf("tty: catch TIOCSWINSZ\n");
		r = sys_vircopy(m_ptr->IO_ENDPT, (vir_bytes)m_ptr->ADDRESS,
				SELF, (vir_bytes)&tp->tty_winsize, (vir_bytes)size);
		sigchar(tp, SIGWINCH);
		break;
	...
	case TIOCGETP:
		...
		r = sys_vircopy(SELF, (vir_bytes)&param.sg,
				m_ptr->IO_ENDPT, (vir_bytes)m_ptr->ADDRESS,
				(vir_bytes)size);
		break;
	...
	case TIOCSETP:
		r = sys_vircopy(m_ptr->IO_ENDPT, (vir_bytes)m_ptr->ADDRESS,
				SELF, (vir_bytes)&param.sg, (vir_bytes)size);
		if (r != OK)
			break;
	...
	case TIOCGETC:
		...
		r = sys_vircopy(SELF, (vir_bytes)&param.tc,
				m_ptr->IO_ENDPT, (vir_bytes)m_ptr->ADDRESS,
				(vir_bytes)size);
		break;
	...
	case TIOCSETC:
		r = sys_vircopy(m_ptr->IO_ENDPT, (vir_bytes)m_ptr->ADDRESS,
				SELF, (vir_bytes)&param.tc, (vir_bytes)size);
		if (r != OK)
			break;
...
PUBLIC int select_retry(struct tty *tp)
{
	...
	{
		printf("tty: select_retry: notifying to %d\n", tp->tty_select_proc);
 ...
 */
{
	...
	do
	{
		...
		if (tp->tty_ioreq != 0)
		{
			printf("tty: tp->tty_ioreq = 0x%08X\n", tp->tty_ioreq);
			dev_ioctl(tp);
		}
	} while (tp->tty_events);

	/* Transfer characters from the input queue to a waiting process. */
	printf("tty: About to call in_transfer\n");
	in_transfer(tp);
	printf("tty: After in_transfer\n");
	...
	if (tp->tty_incum >= tp->tty_min && tp->tty_inleft > 0)
	{
		...
		{
			printf("tty: Notifying to %d\n", tp->tty_incaller);
			notify(tp->tty_incaller);
			tp->tty_inrevived = 1;
		}
		else
		{
			printf("tty: Replying to %d from %d\n", tp->tty_incaller, tp->tty_inproc);
	...
	if (tp->tty_select_ops)
	{
		printf("tty: Before select_retry\n");
		select_retry(tp);
	}
#if NR_PTYS > 0
	if (ispty(tp))
	{
		printf("tty: Before select_retry_pty\n");
		select_retry_pty(tp);
	}
#endif
}
...
/* Transfer bytes from the input queue to a process reading from a terminal. */
{
		...
		if (!(ch & IN_EOF))
		{
			...
			*bp = ch & IN_CHAR;
			...
			if (++bp == bufend(buf))
			{
				...
				printf("tty: Copying to proc %d\n", tp->tty_inproc);
				sys_vircopy(SELF, (vir_bytes)buf,
					    tp->tty_inproc, tp->tty_in_vir,
	...
	if (bp > buf)
	{
		...
		count = bp - buf;
		sys_vircopy(SELF, (vir_bytes)buf,
			    tp->tty_inproc, tp->tty_in_vir, (vir_bytes)count);
		tp->tty_in_vir += count;
		tp->tty_incum += count;
	}
 ...
 */
{
	...
	for (ct = 0; ct < count; ct++)
	{
		...
		if (tp->tty_termios.c_lflag & ISIG)
		{
			...
			if (ch == tp->tty_termios.c_cc[VINTR] || ch == tp->tty_termios.c_cc[VQUIT])
			{
				sig = SIGINT;
 ...
 *				out_process				     *
 *===========================================================================*/
PUBLIC void out_process(
	tty_t *tp,
	char *bstart, char *bpos, char *bend,	/* start/pos/end of circular buffer */
 ...
 */
{
	...
	{
		if (tp->tty_ioreq == TCSETSF)
		{
			printf("tty: Before tty_icancel\n");
			tty_icancel(tp);
		}
		printf("tty: doing copy from %d\n", tp->tty_ioproc);
		result = sys_vircopy(tp->tty_ioproc, tp->tty_iovir,
				     SELF, (vir_bytes)&tp->tty_termios,
...
/* Initialize tty structure and call device initialization routines. */
{
	...
	for (tp = FIRST_TTY, s = 0; tp < END_TTY; tp++, s++)
	{
		...
		tp->tty_min = 1;
		tp->tty_termios = termios_defaults;
		tp->tty_icancel = tp->tty_ocancel = tty_devnop;
		tp->tty_ioctl = tp->tty_close = tty_devnop;
	...
	if (sigaction(SIGTERM, &sa, NULL) < 0)
		panic("TTY", "sigaction failed", errno);
	if (sigaction(SIGKMESS, &sa, NULL) < 0)
		panic("TTY", "sigaction failed", errno);
	if (sigaction(SIGKSTOP, &sa, NULL) < 0)
		panic("TTY", "sigaction failed", errno);
#endif
#if DEBUG
...
/* Notification for a new kernel message. */
{
	...
	if (kmess.km_size > 0)
	{
		...
		while (bytes > 0)
		{
			tty_echo(tty_addr(NR_CONS), kmess.km_buf[(r % KMESS_BUF_SIZE)]);
			bytes--;
			r++;
		}
...
/* Provide the log device with debug output */
{
	...
	r = OK;
	if (sys_vircopy(SELF, (vir_bytes)&kmess,
			m_ptr->m_source, dst,
			sizeof(kmess)) != OK)
	{
		r = EFAULT;
...
/* Print a string for a server. */
{
	...
	{
		if (sys_vircopy(proc_nr, src++, SELF, (vir_bytes)&c, 1) != OK)
		{
			result = EFAULT;
			break;
		}
		tty_echo(tty_addr(NR_CONS), c);
	}
 ...
 *				compat_getp				     *
 *===========================================================================*/
PRIVATE int compat_getp(tty_t *tp, struct sgttyb *sg)
 ...
 *				compat_getc				     *
 *===========================================================================*/
PRIVATE int compat_getc(tty_t *tp, struct tchars *tc)
 ...
 *				compat_setp				     *
 *===========================================================================*/
PRIVATE int compat_setp(tty_t *tp, struct sgttyb *sg)
 ...
 *				compat_setc				     *
 *===========================================================================*/
PRIVATE int compat_setc(tty_t *tp, struct tchars *tc)
 ...
 *				sgspd2tspd				     *
 *===========================================================================*/
PRIVATE speed_t sgspd2tspd(int sgspd)
