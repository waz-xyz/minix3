...
FORWARD void extpartition(struct driver *dp, int extdev, unsigned long extbase);
FORWARD int get_part_table(struct driver *dp, int device, unsigned long offset, struct part_entry *table);
 ...
 */
{
	...
	if ((dv = (*dp->dr_prepare)(device)) == NIL_DEV || dv->dv_size == 0)
		return;
	base = dv->dv_base / SECTOR_SIZE;
	limit = base + dv->dv_size / SECTOR_SIZE;
	...
	for (par = 0; par < NR_PARTITIONS; par++, dv++)
	{
		...
		if (pe->lowsec < base)
			pe->lowsec = base;
		if (part_limit < pe->lowsec)
			part_limit = pe->lowsec;

		dv->dv_base = pe->lowsec * SECTOR_SIZE;
		dv->dv_size = part_limit - pe->lowsec * SECTOR_SIZE;
 ...
 */
{
	...
	do
	{
		if (!get_part_table(dp, extdev, offset, table))
			return;
		sort(table);
		...
		*/
		...
		for (par = 0; par < NR_PARTITIONS; par++)
		{
			...
			else if (pe->sysind != NO_PART)
			{
				if ((dv = (*dp->dr_prepare)(subdev)) == NIL_DEV)
					return;

				dv->dv_base = (extbase + offset + pe->lowsec) * SECTOR_SIZE;
				dv->dv_size = pe->size * SECTOR_SIZE;

				/* Out of devices? */
				if (++subdev % NR_PARTITIONS == 0)
					return;
			}
		}
...
/* Sort a partition table. */
{
	...
	do
	{
		...
		{
			if (pe[0].sysind == NO_PART || (pe[0].lowsec > pe[1].lowsec && pe[1].sysind != NO_PART))
			{
				tmp = pe[0];
				pe[0] = pe[1];
				pe[1] = tmp;
