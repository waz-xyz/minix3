...
FORWARD void init_buffer(void);
FORWARD int do_rdwt(struct driver * dr, message *mp);
FORWARD int do_vrdwt(struct driver * dr, message *mp);
...
/* Main program of any device driver task. */
{
	...
	while (TRUE)
	{
		...
		case DEV_CLOSE:
			...
			break;
		case DEV_IOCTL:
			r = (*dp->dr_ioctl)(dp, &mess);
			break;
		case CANCEL:
			r = (*dp->dr_cancel)(dp, &mess);
			break;
		case DEV_SELECT:
			r = (*dp->dr_select)(dp, &mess);
			break;
		case DEV_READ:
		case DEV_WRITE:
			r = do_rdwt(dp, &mess);
 ...
 */
{
	...
	tmp_buf = buffer;
	sys_umap(SELF, (vir_bytes)buffer, (phys_bytes)sizeof(buffer), &tmp_phys);
...
/* Carry out a single read or write request. */
{
	...
	sys_umap(mp->IO_ENDPT, (vir_bytes)mp->ADDRESS, mp->COUNT, &phys_addr);
	if (phys_addr == 0)
		return EFAULT;

	/* Prepare for I/O. */
	if ((*dp->dr_prepare)(mp->DEVICE) == NIL_DEV)
		return ENXIO;
...
/* Carry out a partition setting/getting request. */
{
	...
	if ((dv = (*dp->dr_prepare)(mp->DEVICE)) == NIL_DEV)
		return ENXIO;
