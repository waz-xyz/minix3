...
FORWARD char *log_name(void);
FORWARD struct device *log_prepare(int device);
FORWARD int log_transfer(int proc_nr, int opcode, off_t position, iovec_t *iov, unsigned nr_req);
FORWARD int log_do_open(struct driver *dp, message *m_ptr);
FORWARD int log_cancel(struct driver *dp, message *m_ptr);
FORWARD int log_select(struct driver *dp, message *m_ptr);
FORWARD void log_signal(struct driver *dp, message *m_ptr);
FORWARD int log_other(struct driver *dp, message *m_ptr);
FORWARD void log_geometry(struct partition *entry);
FORWARD int subread(struct logdevice *log, int count, int proc_nr, vir_bytes user_vir);
...
PUBLIC int main(void)
{
	...
	for (i = 0; i < NR_DEVS; i++)
	{
		log_geom[i].dv_size = LOG_SIZE;
		log_geom[i].dv_base = (uint64_t)logdevices[i].log_buffer;
 ...
 *				log_prepare				     *
 *===========================================================================*/
PRIVATE struct device *log_prepare(device) int device;
{
...
subwrite(struct logdevice *log, int count, int proc_nr, vir_bytes user_vir)
{
	...
	{
		if ((r = sys_vircopy(proc_nr, user_vir, SELF, (vir_bytes)buf, count)) != OK)
			return r;
	}
...
subread(struct logdevice *log, int count, int proc_nr, vir_bytes user_vir)
{
	...
	if ((r = sys_vircopy(SELF, (vir_bytes)buf, proc_nr, user_vir, count)) != OK)
		return r;
 ...
 *				log_transfer				     *
 *===========================================================================*/
PRIVATE int log_transfer(proc_nr, opcode, position, iov, nr_req) int proc_nr; /* process doing the request */
...
unsigned nr_req;							      /* length of request vector */
{
	...
	dv_size = (unsigned long)(dv->dv_size);
 ...
 *				log_do_open				      *
 *============================================================================*/
PRIVATE int log_do_open(dp, m_ptr) struct driver *dp;
message *m_ptr;
{
	if (log_prepare(m_ptr->DEVICE) == NIL_DEV)
		return (ENXIO);
	return (OK);
}
 ...
 *				log_geometry				      *
 *============================================================================*/
PRIVATE void log_geometry(entry) struct partition *entry;
{
	/* take a page from the fake memory device geometry */
	entry->heads = 64;
	entry->sectors = 32;
	entry->cylinders = log_geom[log_device].dv_size / SECTOR_SIZE /
 ...
 *				log_cancel				      *
 *============================================================================*/
PRIVATE int log_cancel(dp, m_ptr) struct driver *dp;
message *m_ptr;
{
	int d;
	d = m_ptr->TTY_LINE;
...
PRIVATE void do_status(message *m_ptr)
{
	...
	for (d = 0; d < NR_DEVS; d++)
	{
		...
		if (logdevices[d].log_proc_nr && logdevices[d].log_revive_alerted && logdevices[d].log_source == m_ptr->m_source)
		{
			m.m_type = DEV_REVIVE;
		...
		if (logdevices[d].log_selected && logdevices[d].log_select_proc == m_ptr->m_source && logdevices[d].log_select_alerted)
		{
 ...
 *				log_signal				      *
 *============================================================================*/
PRIVATE void log_signal(dp, m_ptr) struct driver *dp;
message *m_ptr;
{
 ...
 *				log_other				      *
 *============================================================================*/
PRIVATE int log_other(dp, m_ptr) struct driver *dp;
message *m_ptr;
{
	int r;
 ...
 *				log_select				      *
 *============================================================================*/
PRIVATE int log_select(dp, m_ptr) struct driver *dp;
message *m_ptr;
{
