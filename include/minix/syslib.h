...
int sys_exec(int proc, char *ptr, char *aout, vir_bytes initpc);
int sys_fork(int parent, int child, int *);
int sys_newmap(int proc, struct mem_map *ptr);
int sys_exit(int proc);
int sys_trace(int req, int proc, long addr, long *data_p);

int sys_privctl(int proc, int req, int i, void *p);
int sys_nice(int proc, int priority);
...
int sys_sdevio(int req, long port, int type, int proc_nr, void *buffer, int count);
...
/* Signal control. */
int sys_kill(int proc, int sig);
int sys_sigsend(int proc_nr, struct sigmsg *sig_ctxt); 
int sys_sigreturn(int proc_nr, struct sigmsg *sig_ctxt);
int sys_getksig(int *k_proc_nr, sigset_t *k_sig_map); 
