 ...
 * responsible for handling the call. By conventionsys_call() is transformed 
 * into a message with type SYS_CALL that is handled in a function do_call().
 * 
 * Changes:
 *   Jul 302005   created SYS_INT86 to support BIOS driver  (Philip Homburg)
 *   Jul 132005   created SYS_PRIVCTL to manage services  (Jorrit N. Herder)
 *   Jul 092005   updated SYS_KILL to signal services  (Jorrit N. Herder)
 *   Jun 212005   created SYS_NICE for nice(2) kernel call  (Ben J. Gras)
 *   Jun 212005   created SYS_MEMSET to speed up exec(2)  (Ben J. Gras)
 *   Apr 122005   updated SYS_VCOPY for virtual_copy()  (Jorrit N. Herder)
 *   Jan 202005   updated SYS_COPY for virtual_copy()  (Jorrit N. Herder)
 *   Oct 242004   created SYS_GETKSIG to support PM  (Jorrit N. Herder)
 *   Oct 102004   created handler for unused calls  (Jorrit N. Herder)
 *   Sep 092004   updated SYS_EXIT to let services exit  (Jorrit N. Herder)
 *   Aug 252004   rewrote SYS_SETALARM to clean up code  (Jorrit N. Herder
 *   Jul 132004   created SYS_SEGCTL to support drivers  (Jorrit N. Herder)
 *   May 242004   created SYS_SDEVIO to support drivers  (Jorrit N. Herder)
 *   May 242004   created SYS_GETINFO to retrieve info  (Jorrit N. Herder)
 *   Apr 182004   created SYS_VDEVIO to support drivers  (Jorrit N. Herder)
 *   Feb 242004   created SYS_IRQCTL to support drivers  (Jorrit N. Herder)
 *   Feb 022004   created SYS_DEVIO to support drivers  (Jorrit N. Herder)
 */
