 ...
 */
FORWARD int mini_send(struct proc *caller_ptr, int dst_e, message *m_ptr, unsigned flags);
FORWARD int mini_receive(struct proc *caller_ptr, int src, message *m_ptr, unsigned flags);
FORWARD int mini_notify(struct proc *caller_ptr, int dst);
FORWARD int deadlock(int function, struct proc *caller, int src_dst);
FORWARD void enqueue(struct proc *rp);
FORWARD void dequeue(struct proc *rp);
...
PRIVATE int deadlock(
	...
	struct proc *cp,		/* pointer to caller */
...
PRIVATE int mini_receive(
	struct proc *caller_ptr,	/* process trying to get message */
...
#endif
				if (src_e!=ANY && src_p != src_proc_nr)
 ...
 *===========================================================================*/
PRIVATE void sched(
	struct proc *rp,		/* process to be scheduled */
 ...
 */
{
	 ...
	 * lowest queue.  
	 */
	if (time_left == 0)				/* quantum consumed ? */
	{
	...
	*p = _ENDPOINT_P(e);
	...
	else if (proc_addr(*p)->p_endpoint != e)
	{
		...
		kprintf("kernel:%s:%d: bad endpoint %d: proc %d has ept %d (generation %d vs. %d)\n",
			file, line, e, *p, proc_addr(*p)->p_endpoint,
