 ...
 */
{
	...
	if ((vec_size = m_ptr->DIO_VEC_SIZE) <= 0)
		return EINVAL;
	...
	case DIO_LONG:
		...
		break;
	default:
		return EINVAL;	/* check type once and for all */
	}
	if (bytes > sizeof(vdevio_buf))
		return E2BIG;
	...
	if (io_in)
		phys_copy(vir2phys(vdevio_buf), caller_phys, (phys_bytes)bytes);
