 ...
 */
{
	...
	if (!isokendpt(tr_proc_nr_e, &tr_proc_nr))
		return EINVAL;
	if (iskerneln(tr_proc_nr))
		return EPERM;
	...
	case T_GETINS: /* return value from instruction space */
		...
		{
			if ((src = umap_local(rp, T, tr_addr, TR_VLSIZE)) == 0)
				return EIO;
	...
	case T_GETDATA: /* return value from data space */
		if ((src = umap_local(rp, D, tr_addr, TR_VLSIZE)) == 0)
			return EIO;
		phys_copy(src, vir2phys(&tr_data), (phys_bytes)sizeof(long));
		m_ptr->CTL_DATA = tr_data;
		break;
	...
	case T_SETINS: /* set value in instruction space */
		if (rp->p_memmap[T].mem_len != 0)
		{
			if ((dst = umap_local(rp, T, tr_addr, TR_VLSIZE)) == 0)
				return EIO;
	...
	case T_SETDATA: /* set value in data space */
		if ((dst = umap_local(rp, D, tr_addr, TR_VLSIZE)) == 0)
			return EIO;
		phys_copy(vir2phys(&tr_data), dst, (phys_bytes)sizeof(long));
		m_ptr->CTL_DATA = 0;
		break;
