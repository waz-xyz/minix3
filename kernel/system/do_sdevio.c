...
)
{
	...
	if (proc_nr_e == SELF)
		proc_nr = who_p;
	else if (!isokendpt(proc_nr_e, &proc_nr))
		return EINVAL;
	...
	if (m_ptr->DIO_REQUEST == DIO_INPUT)
	{
		...
		{
		case DIO_BYTE:
			phys_insb(port, phys_buf, count);
			break;
		case DIO_WORD:
			phys_insw(port, phys_buf, count);
			break;
		default:
			return EINVAL;
		}
	...
	else if (m_ptr->DIO_REQUEST == DIO_OUTPUT)
	{
		...
		{
		case DIO_BYTE:
			phys_outsb(port, phys_buf, count);
			break;
		case DIO_WORD:
			phys_outsw(port, phys_buf, count);
			break;
		default:
			return EINVAL;
		}
