...
int main(int argc, char *argv[])
{
	 ...
	 * want, then go.
	 */
	current_time = time(NULL); /* time mkfs is being run */
	stat(argv[0], &statbuf);
	...
	{
		if (!block_size)
			block_size = _MAX_BLOCK_SIZE; /* V3 default block size */
	...
	if (inodes_per_block == 0)
	{
		...
		printf("!!!! Setting inodes_per_block = %d\n", inodes_per_block);
	}
	...
	optarg = argv[++optind];
	printf("main: start processing proto %s\n", optarg);
	...
	printf("!!!! Adding root directory\n");
	root_inum = alloc_inode(mode, usrid, grpid);
	rootdir(root_inum);
	if (simple == 0)
	{
		printf("!!!! Procesing proto\n");
		eat_dir(root_inum);
	}
...
void super(zone_t zones, ino_t inodes)
{
	...
	for (cp = buf; cp < &buf[block_size]; cp++)
		*cp = 0;
	...
	printf("inodes = %u, inodes_per_block = %u\n", inodes, inodes_per_block);
...
#define MAX_MAX_SIZE ((unsigned long)0xffffffffUL)
...
void rootdir(ino_t inode)
{
	...
	z = alloc_zone();
	printf("!!!! Adding zone %lu to inode %lu\n", z, inode);
	add_zone(inode, z, 2 * sizeof(struct direct), current_time);
	printf("!!!! Adding entry '.' to inode %lu\n", inode);
...
void get_line(line, parse) char *parse[MAX_TOKENS];
char line[LINE_LEN];
{
...
void check_mtab(char *devname /* /dev/hd1 or whatever */)
{
	...
	printf("check_mtab: doing stat on %s\n", devname);
	r = stat(devname, &sb);
	if (r == -1)
	{
		if (errno == ENOENT)
...
void special(char *string)
{
	printf("special: creating special %s\n", string);
